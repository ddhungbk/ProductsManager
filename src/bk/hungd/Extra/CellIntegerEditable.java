 ...
 * {@link TextField} node inside the cell. If the TextField is left, the value
 * is commited.
 * 
 */

public class CellIntegerEditable<S, T> extends TableCell<S, T> {
	...
	public static <S, T> Callback<TableColumn<S, T>, TableCell<S, T>> forTableColumn(
		...
		return list -> new CellIntegerEditable<S, T>(converter);
	}
	 ...
	 *            T.
	 */
	public CellIntegerEditable(StringConverter<T> converter) {
	...
	public void commitEdit(T newValue) {
		...
		if (table != null) {
			...
			// clicked out of the table entirely and given focus to something
			// else.
	...
	public void cancelEdit() {
		if (escapePressed) {
			...
			String newText = textField.getText(); // get the new text from the
													// view
			this.commitEdit(getConverter().fromString(newText)); // commit the
																	// new text
																	// to the
																	// model
		}
	...
	private TextField getTextField() {
		...
		textField.setOnKeyReleased(t -> {
			if (t.getCode() == KeyCode.ESCAPE) {
				// djw the code may depend on java version / expose
				// incompatibilities:
